    ...
    <div className="h-full flex flex-col p-6">
      ...
      <div className="flex-1 min-h-0 flex gap-4">
        ...
        <div className="flex-1 min-w-0 flex flex-col gap-4">
          ...
          {/* Legend */}
          <div className="flex items-center justify-center gap-8 py-3 px-6 rounded-xl border border-metal/30 bg-void-light/30 backdrop-blur-sm text-[10px] font-code text-steel uppercase tracking-widest">
            <div className="flex items-center gap-2">
              <span className="w-3 h-3 rounded box-border border-2 border-neon-cpp bg-neon-cpp/20" />
              Active
            </div>
            <div className="flex items-center gap-2">
              <span className="w-3 h-3 rounded box-border border-2 border-neon-go bg-neon-go/20" />
              Freed
            </div>
            <div className="flex items-center gap-2">
              <span className="w-3 h-3 rounded box-border border-2 border-red-500 bg-red-500/20 shadow-[0_0_8px_rgba(239,68,68,0.3)]" />
              Leak
            </div>
            <div className="flex items-center gap-2">
              <span className="w-3 h-3 rounded box-border border-2 border-steel bg-steel/10" />
              Garbage
            </div>
            <div className="text-neon-rust ml-4 italic px-2 py-0.5 rounded bg-neon-rust/5">
              Arrows indicate pointers --&gt;
            </div>
          </div>
        </div>

        {/* Concept Guide (new vertical panel) */}
        <div className="w-64 bg-void-light/50 rounded-xl border border-metal/50 p-6 flex flex-col">
          <h4 className="text-[10px] font-code text-steel font-bold mb-4 flex items-center gap-2 uppercase tracking-widest border-b border-metal/30 pb-2">
          ...
          <div className="flex-1 overflow-y-auto space-y-6">
            <ConceptItem 
              ...
              active={language === 'cpp'}
              description="In C++, you must manually 'free' memory. If you forget, it stays allocated forever (a leak) even if you lose the pointer."
            />
            <ConceptItem 
              title="Garbage Collection" 
              active={['python', 'javascript', 'go'].includes(language)}
              description="Languages like Go and Python automatically 'sweep' memory when it becomes unreachable. You don't have to worry about manually freeing it."
            />
            <ConceptItem 
              title="Ownership System" 
              active={language === 'rust'}
              description="Rust ensures safety without GC. Variables 'own' memory, and it's automatically freed the moment the owner goes out of scope."
            />
            <ConceptItem 
              title="Stack vs Heap" 
              active={true}
              description={`The **Stack** is fast and orderly (LIFO), used for temporary variables. The **Heap** is for objects that need to outlive their current scope.`}
            />
          </div>
        </div>
      </div>
